...
CIVARest <- function (res)
{
  ...
  if (type == "rconst")                     Model = "II"
  if (type == "rtrend")                     Model = "IV"
  if (type == "trend")                      Model = "V"
  ...
  param = tst[[2]][[1]]
  CIVAREST = VECM2VAR(param = param, beta = tst$beta, p = c(crk,p - 1, 0))
  B = VECM2VAR(param = param, beta = tst$beta, p = c(crk, p - 1, 0))[[1]]
  C = VECM2VAR(param = param, beta = tst$beta, p = c(crk, p - 1, 0))[[3]]
  ...
  if (type=="rconst")   Co = alpha%*%t(beta)[,n+1]
  if (type=="rtrend")   C1 = alpha%*%t(beta)[,n+1]
  if (type=="trend")  { C1 = param[crk+(p-1)*n+2,] ; Co = param[crk+(p-1)*n+1,] }
  LREG = tst[[2]]
...
#'
#' @export
MRCVECMest2 <-function (y, x, s, model = c("I", "II", "III",
                                           "IV", "V"), type = c("eigen", "trace"),
                        crk = 2, P = matrix(2, 2, 2), q = 0.95, Dxflag = 0, X = NA)
{
  y <- as.matrix(y)
...
###
    ...
    #N  <- ncol(as.matrix(y))
    if (N1<r) {
        print("y's dimension must be larger than r")
        break()
    }
...
#' @param p A vector specifying different type of VECMs
 ...
 if (anyNA(s)) {
 	if (!p[3]==0) {
		...
		B = (1:(m*m*(p[2]+1)))*0; dim(B) = c(m,m,(p[2]+1))
 		...
 		if (p[2]>0) for ( i in 2:(p[2]+1)) B[,,i] = VECB[, p[1]+((i-2)*m+1):((i-2)*m+m)]

 		B <- CIB3B(CIB=B)
      	A = NA
  	}
  	if  (dim(param)[1]> p[1]+(p[2]+p[3])*m ) C = as.matrix(t(param)[,(p[1]+(p[2]+p[3])*m+1):dim(param)[1]]) else C = NA
  } else   {
...
irf_CIVAR_CB <- function (res, nstep, comb, irf = c("gen", "chol", "chol1", "gen1", "comb1"), runs = 200, conf = c(0.05, 0.95))
{
  ...
  for (i in 1:runs) {
    ...
    res_run   = CIVARData(n, p, T, r_np = NA, A=NA, B, Co, C1,U = Uo_run, Sigma = NA, type,X,mu=NA,Yo=NA,crk)
