...
###
    ...
    for (i in 1:n) {
      ...
      LH_BIC[i] = log(sum(2 - St)) * (m * (sum(p[i, 1:2, 1]) -
        ...
        2 * LH_P[i]
      AB1 = VECM2VAR(param = tst$VECM1[[1]][c(1:crk,2*crk +(1: (m * (pj[1,1] - 1 + pj[1, 2] - 1)))), ], beta = tst$betaS, p = c(crk,(pj[1, 1] - 1), (pj[1, 2] - 1)))
      AB2 = VECM2VAR(param = tst$VECM1[[1]][c(crk+(1:crk), (crk*2 + m * (pj[1, 1] - 1 + pj[1, 2] - 1)) + 1:(m * (pj[2,1] + pj[2, 2] - 2))), ], beta = tst$betaS, p = c(crk,(pj[2, 1] - 1), (pj[2, 2] - 1)))
