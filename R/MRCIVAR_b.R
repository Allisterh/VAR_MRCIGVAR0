...
#' @param p     : an (S x 2) matrix. Each row of p contains the lag length for the corresponding state and the number of exogenous variables for the state.
...
#' @export
MRCVECMestm <- function (y, x, s, model = c("I", "II", "III", "IV", "V"), type = c("eigen", "trace"),
                         crk = 2, P = matrix(2, 2, 2), q = 0.95, Dxflag = 0, X = X)
{
  y <- as.matrix(y)
...
#'   GIRF <- girf_MRCIVAR_RM_CB(res=res_e,shock=c(1,1,1,1),R=100,nstep=10,Omega_hist=NA,resid_method="parametric",conf_level=c(0.05,0.95),N=100)
