...
This is a brief overview of the MRCIGVAR package. This version of MRCIGVAR can be found on GitHub at https://github.com/puchen8229/MRCIGVAR0.
...
This package includes functions for data generation with flexible model specifications, parameter estimation, model selection, impulse response functions, and parameter constraint tests for each model class. Using sample code, this introduction demonstrates how to use the functions in practical applications.
...
The following codes demonstrate how VARData is used to generate data from a VAR process with a variety of specifications.
```{r,results='hide'}
...
The VARest dunction is used to estimate the parameters for the given data.
...
$$Y_t = C_0 + B_1Y_{t-1} + B_2Y_{t-1} + ... + B_pY_{t-p} + u_t$$
Because of unit roots and cointegration relations in the VAR process, it has the following vector error correction form:
...
The default number of unit-root is one. The option of r_np matrix can be used to specify the number of unit roots.
...
B0 = res_d$B
res_d  = CIVARData(n=4,p=3,T=500,B = B0,Co=matrix(0,n,1) ,type="none",crk=3)
res_e  = CIVARest(res=res_d)
sum(abs(res_e$B-res_d$B))
...
```
The CIVARTest function calculates maximum likelihood tests on the restrictions within the cointegration space i.e. restrictions on $\beta$ and $\alpha$.
...
CCIVARData and CCIVARest are functions used to model conditional cointegrated VAR processes. [^1] 
...
girf_MRVAR_RM_CB can be used to produce the generalized impulse response function with regime migrations.[^2] irf_MRVAR_NM_CB is used to generate the within-regime impulse response functions [^3]
...
The following codes demonstrate how AIC and BIC criteria can be used in the model specification.
...
+ Replace the generated model data with the empirical data.
+ Estimate the model parameters using MRVARest
...
$$
where $\alpha^{(1)}$, $\alpha^{(2)}$ and $\beta$ are $(n\times ckr)$ matrices. We assume the cointegration relations are identical in different regimes while the adjustment speed can be regime-dependnet.
...
MRCIVARDatam generates data from a two-regime cointegrated VAR process with identical cointegration relations and different adjustment speeds in the two regimes. The following sample codes demonstrate how MRCIVARDatam can be used to generate data with a variety of specifications and how MRCIVARestm is used to estimate the parameters. MRCIVAR_Selectm calculates AIC or BIC criteria for a range of parameter settings.
```{r,results='hide',fig.show='hide'}
Sigma = 1:(4*4*2)
dim(Sigma) = c(4,4,2)
Sigma[,,1] = diag(4)
...
res_e$Summary
```
The output of MRCIVARDatam is an MRCIVAR object containing generated data and parameters as well as the residuals. The components of the generated MRCIVAR object correspond to the model variables and parameters as follows.
...
STAT(res_d$Bo[,,,1])
STAT(res_d$Bo[,,,2])
## Johansen test of the cointegration rank
res_e$tst$erg
...
where $y_{i,t}$ is an $m$-vector of domestic variables with $y_{i,t}=(y_{i,1,t},y_{i,2,t},...,y_{i,m,t})'$ and $y_{i,t}^{*}$ is an $m$-vector of the corresponding foreign variables with $y_{i,t}^{*}=(y_{i,1,t}^{*},y_{i,2,t}^{*},...,y_{i,m,t}^{*})'$,  $i=1,2,...,n$. $n$ is the number of countries. Stacking all $n$ country models together we have the GVAR in its global form:
$$
...
In the country regression output, Estimation_Result[[5]] indicates this is the output of country 5. Y1.l1, Y2.l1, Y2.l2, FY1.l1 FY2.1 correspond to the domestic variable and the foreign variables:  $Y_{5,1,t-1}$, $Y_{5,2,t-1}$, $Y_{5,1,t-2}$, $Y_{5,2,t-2}$, $Y_{5,1,t-2}^{*}$, $Y_{5,2,t-1}^{*}$,respectively. 
...
```
As the package allows a country-wise specification of the lag length. Information criteria can be applied to select the model country by country to obtain a proper specification of the lags in a  GVAR model. GVAR_Select calculates AIC and BIC to lags up to the upper bounds given in L_V for country I.
...
CIGVARData creates a CIGVAR object with provided or randomly generated parameters and simulated data. CIGVARData1 creates a CIGVAR object with common stochastic trends and country specific idiosyncratic stochastic trends. CIGVARest estimates the parameters of a CIGVAR object.   
...
res_d\$Y:      $(y_{1,1},y_{1,2},...,y_{1,m},y_{2,1},y_{2,2},...y_{2,m},...,y_{n,m})$

res_d\$Uo:      $(\epsilon_{1,1},\epsilon_{1,2},...,\epsilon_{1,m},\epsilon_{2,1},\epsilon_{2,2},...\epsilon_{2,m},...,\epsilon_{n,m})$
...
p = (1:15)*0; dim(p) = c(5,3)
p[,1] = 3; p[,2]=3; p[,3] = 1
res_d = GVARData(m=2,n=5,p=p,T=200,type="const")
...
res_d = CIGVARData1(m=2,n=5,p=p,T=200,type="exog0",X=X,d=0,Ncommtrend = 1)
max(res_d$Y)
plot(ts(res_d$Y))
...
res_d = CIGVARData1(m=2,n=5,p=p,T=200,type="exog1",X=X,d=0,Ncommtrend = 1)
max(res_d$Y)
plot(ts(res_d$Y))
res_e = CIGVARest(res=res_d)
res_e$Summary
...
pp
```
The example above shows how GVAR_Select is used for the model selection. The data generating process has a lag length of 3 for every country's domestic variables and the foreign variables as well. The information criteria selection is very close to the true specification.
...
$$ 
where $y_{i,t}$ is an $m$-vector of domestic variables and $y_{i,t}^{*}$ is an $m$-vector of the corresponding foreign variables. $i=1,2,...,n$. $n$ is the number of countries. $S_{i,t}$ is a state variable that indicates in which state country $i$ is at time $t$. Stacking all $n$ country models together we have the MRGVAR in its global form:
$$
...
IRF_CB  = irf_MRGVAR_CB1(res=res_e,nstep=10,comb=NA,state=rep(1,10),irf="gen1",runs=20,conf=c(0.05,0.95))
...
```
MRGVARest is used to estimate the parameters of an MRGVAR object with provided data. irf_MRGVAR_CB1 and girf_MRGVAR_RM_CB generate regime specific impulse response functions without regime migration and generalized impulse response functions with regime migration, respectively. 

### MRCIGVAR

A multi regime cointegrated global vector autoregressive model is an MRGVAR with unit roots in every country. Each county model is assumed to have a vector error correction form:
...
where $y_{i,t}$ is an $m$-vector of domestic variables and $y_{i,t}^{*}$ is an $m$-vector of the corresponding foreign variables. $i=1,2,...,n$. $n$ is the number of countries. $S_{i,t}$ is a state variable that indicates which state country $i$ at time $t$ is in. $S_{i,t}=1$ for $f_{1,t-d} \le \tau_i.$ and $S_{i,t}=2$ for $f_{1,t-d} > \tau_i$. The country model in level of the variables is:
...
The impulse response function options comb and comb1 allow you to calculate global shocks, regional shocks, coordinated actions, and generate the system's global and regional responses.
...
We define concerted financial stress shocks in state $S$: $v_{t,S}^{cf}=(v_{1},0,v_{3},0,...,v_{2N-1},0)$,
